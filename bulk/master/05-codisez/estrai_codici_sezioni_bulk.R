#####################################################################
# SERVIZIO MASSIVO PER ASSOCIAZIONE A COORDINATE DEL CODICE SEZIONE #
#####################################################################

t0 <- Sys.time()
masteRfun::load_pkgs('data.table') |> invisible()
...
# fn <- file.path(percorsi$smb, 'master', 'bulk', 'codisez', 'invio', 'test.csv')
...
yr <- y |> masteRgeo::trova_codici_micro(omi = FALSE, cap = FALSE, utb = FALSE)
